...
class TriodosPipeline:

    # Database setup
    conn = sqlite3.connect('triodos.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `triodos`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM triodos WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO triodos (date, title, link, content)"
